...
Basically, RedHat broke Perl by setting LANG to UTF8
They apparently have a fixed version in RawHide.

You 
might try running CPAN or creating your Makefile after exporting the 
...
=item examples/unzipAll.pl -- uses tree operations to unzip an entire Zip
...
Archive::Zip v1.02 added the archive methods overwrite() and overwriteAs() to
do this simply and carefully.
...
Indeed, many systems don't support such a concept.
...
Depending on the format of the gzip file, you can use Compress::Zlib, or
...
for their 'eocdOffset' property, which should be 0:
  ...
  if ($zip->eocdOffset > 0)
    { warn($zip->eocdOffset . " bytes of garbage at beginning of Zip") }
...
=head1 I can't compile but AS only has an old version of Archive::Zip
