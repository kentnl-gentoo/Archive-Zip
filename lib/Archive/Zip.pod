...
=item Archive::Zip::useSymbolicLinks( [$wants] )

Get or set the status of C<useSymbolicLinks>. If set to a
true value, on systems that support symbolic links they
will be read and stored instead of the file data.

Default is to not store symbolic links.
