...
To use absolute pathnames, just pass them in: $zip->addTree(
'/a/b', '/a/b' );
...
To translate relative to absolute pathnames, just pass them
in: $zip->addTree( '.', '/c/d' );
...
files in the zip with their original names. If you give
arguments, C<extractTree> extracts all the members whose path
names start with C<$root>. Will translate that root to a
given dest pathname. For instance,
